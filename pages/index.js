...
const BASEURL =
    'https://raw.githubusercontent.com/LuganoStats/LVGAPoint-Dataset/dataset'

export async function getStaticProps(context) {
    let lastMonth
    await fetch(`${BASEURL}/last/month.json`)
        .then((el) => el.json())
        .then((el) => getDayTransactionCount(el))
        .then((el) => (lastMonth = el))

    let last3Month
    await fetch(`${BASEURL}/last/3month.json`)
        .then((el) => el.json())
        .then((el) => getDayTransactionCount(el))
        .then((el) => (last3Month = el))

    let last6Month
    await fetch(`${BASEURL}/last/6month.json`)
        .then((el) => el.json())
        .then((el) => getDayTransactionCount(el))
        .then((el) => (last6Month = el))

    let lastyear
    await fetch(`${BASEURL}/last/year.json`)
        .then((el) => el.json())
        .then((el) => getDayTransactionCount(el))
        .then((el) => (lastyear = el))
